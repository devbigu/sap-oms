...
test("ledger booked debit excludes old orders with numeric or string identifiers", () => {
  const scoped = ledger.ordersForDealer([oldOrder, activeOrder], dealerId);
  assert.deepEqual(scoped.map((order) => String(order.order_id)), ["13001"]);
...
test("historical manual payments remain credits even when all related orders are before cutoff", () => {
  ...
  assert.equal(bookedPaise, 0);
  assert.equal(historicalPaymentPaise, 8_000_000);
  assert.equal((bookedPaise - historicalPaymentPaise) / 100, -80000);
});

test("cached ledger snapshots are versioned and re-filter old orders on read", async () => {
  const source = await fs.readFile(path.resolve("src/lib/ledgerSystem.ts"), "utf8");
  assert.match(source, /collective_ledger_snapshot:\$\{ACTIVE_ORDER_PERIOD_VERSION\}/);
  assert.match(source, /orders:\s*filterActiveOrders\(Array\.isArray\(doc\.orders\)/);
});
