...
const NAV: Record<Role, NavItem[]> = {
  ...
  dealer: [
    ...
    { section: "Products", label: "Products", href: "/Products", icon: <Package size={15} /> },
  ],
  staff: [
...
export default function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const badge =
    role === "dealer"
      ? session?.dealerCode
      : role === "staff"
        ? staffRoleLabel(session?.staffRoleType)
        : role === "accountant"
          ? "Accountant"
          : role === "admin"
            ? "Administrator"
    ...
    <>
      ...
      <aside className={`sb-panel${open ? " open" : ""}`}>
        ...
        <div className="sb-user">
          <div className="sb-avatar">{loading ? ".." : getInitials(name)}</div>
          <div className="sb-uname">{loading ? "Loading..." : name ?? "Authenticated user"}</div>
          <div className="sb-meta">{loading ? "" : meta}</div>
          {!loading && badge && <span className="sb-role">{badge}</span>}
        </div>
